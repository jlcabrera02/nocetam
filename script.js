...
const app = (function () {
  ...
  public.createNote = function (coor) {
    ...
    contextNota.fillText(`Tiempo: ${zfill(day, 2)}-${zfill(month, 2)}-${zfill(year, 4)} ${zfill(hour, 2)}:${zfill(minute, 2)}`, 5, 300);
  ...
  public.aplastarNota = function(urlImage){
    ...
    let pitagoras = (Math.sqrt(Math.pow(edit.swidth, 2) + Math.pow(edit.sheight, 2)))/6
    
    let relacionAspecto = 380/820,
    newHeight = relacionAspecto*pitagoras;
 
    nota.onload = function () {
      context.drawImage(nota, 8, edit.sheight-(newHeight+5), pitagoras*2.4, newHeight)
    }
  }
